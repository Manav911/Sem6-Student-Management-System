...
<body>
	...
	<fm:form modelAttribute="subList" name="/subList">
		<div class="w3-table w3-blue">
			<div
				...
				<div class="rbutton">
					<c:if test="${empty id}">
						<a href="/loadSubjectPage"><button
								class="btn btn--radius-2 btn btn-danger" type="button">New
								Subject</button></a>
					</c:if>
					<c:if test="${not empty id}">
						<a href="/faculty/loadSubjectPage/${id}"><button
								class="btn btn--radius-2 btn btn-danger" type="button">New
								Subject</button></a>
					</c:if>
				</div>
				...
				<thead align="center" title="Subject List">
					...
					<c:forEach items="${subList}" var="su">
						<tr>
							<td>${su.subjectid}</td>
							<td>${su.subname}</td>
							<td>${su.substandard}</td>
							<td><c:if test="${empty id}">
									<a href="/deleteSubject/${su.subjectid}"><button
											type="button" class="btn btn-primary">Delete</button> </a>
								</c:if> <c:if test="${not empty id}">
									<a href="/faculty/deleteSubject/${su.subjectid}/${id}"><button
											type="button" class="btn btn-primary">Delete</button> </a>
								</c:if></td>
						</tr>
					</c:forEach>
