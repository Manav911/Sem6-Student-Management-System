...
<body>
	...
	<fm:form modelAttribute="assignmentList" name="/assignmentList"
		...
		<div class="w3-table w3-blue">
			<div
				...
				<div class="rbutton">
					<c:if test="${empty id}">
						<a href="/loadAssignmentUpload"><button
								class="btn btn--radius-2 btn btn-danger" type="button">New
								Assignment</button></a>
					</c:if>
					<c:if test="${not empty id}">
						<a href="/faculty/loadAssignmentUpload/${id}"><button
								class="btn btn--radius-2 btn btn-danger" type="button">New
								Assignment</button></a>
					</c:if>
				</div>
			...
			<table class="table table-striped mt-3">
				<tbody>
				<thead align="center" title="Assignment List">
					<tr>
						<th scope="col">ID</th>
					...
					<c:forEach items="${assignmentList}" var="assList">
						<tr>
							<td>${assList.assid}</td>
							<td>${assList.asstitle}</td>
							<td>${assList.assdescription}</td>
							<td>${assList.standard}</td>
							<td>${assList.classname}</td>
							<td>${assList.givenbyname}</td>
							<c:if test="${empty id}">
								<td><a href="/deleteAssignment/${assList.assid}"><button
											type="button" class="btn btn-primary">Delete</button></a></td>
							</c:if>
							<c:if test="${not empty id}">
								<td><a href="/faculty/deleteAssignment/${assList.assid}/${id}"><button
											type="button" class="btn btn-primary">Delete</button></a></td>
							</c:if>
