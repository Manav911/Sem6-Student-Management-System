...
<body>
	<h1 align="center">Standard List</h1>
	<fm:form modelAttribute="stdList" name="/stdList">
		<div class="w3-table w3-blue">
			<div
				...
				<div class="rbutton">
					<c:if test="${empty id}">
						<a href="/loadStandardPage"><button
								class="btn btn--radius-2 btn btn-danger" type="button">New
								Standard</button></a>
					</c:if>
					<c:if test="${not empty id}">
						<a href="/faculty/loadStandardPage/${id}"><button
								class="btn btn--radius-2 btn btn-danger" type="button">New
								Standard</button></a>
					</c:if>
				</div>
			...
			<table class="table table-striped mt-3">
				<tbody>
				<thead align="center" title="Subject List">
					<tr>
					...
					<c:forEach items="${stdList}" var="st">
						<tr>
							<td>${st.stid}</td>
							<td>${st.standard}</td>
							<td><c:if test="${empty id}">
									<a href="/deleteStandard/${st.stid}"><button
											type="button" class="btn btn-primary">Delete</button> </a>
								</c:if> <c:if test="${not empty id}">
									<a href="/faculty/deleteStandard/${st.stid}/${id}"><button
											type="button" class="btn btn-primary">Delete</button> </a>
								</c:if></td>
						</tr>
					</c:forEach>
